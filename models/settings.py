...
class ResConfigSettings(models.TransientModel):
    ...
    account_payable_id = fields.Many2one(comodel_name="account.account", string="Account Payable",config_parameter='base_setup.account_payable_id', )
    account_receive_id = fields.Many2one(comodel_name="account.account", string="Account Receive",config_parameter='base_setup.account_receive_id', )
    warehouse_id = fields.Many2one(comodel_name="stock.warehouse", string="Test Warehouse",config_parameter='base_setup.warehouse_id', default_model="sale.order")
    sample_warehouse_id = fields.Many2one(comodel_name="stock.warehouse", string="Sample Warehouse",config_parameter='base_setup.sample_warehouse_id', )
    examination_warehouse_id = fields.Many2one(comodel_name="stock.warehouse", string="examination  Warehouse",config_parameter='base_setup.examination_warehouse_id', )
    outsource_warehouse_id = fields.Many2one(comodel_name="stock.warehouse", string="OutSource Warehouse",config_parameter='base_setup.outsource_warehouse_id', )
    call_center_token = fields.Char(string="Call Center Token Key", config_parameter='base_setup.call_center_token', )
    call_center_token_len = fields.Integer(string="callcenter Token Length",)
    ...
    magento_token_len = fields.Integer(string="Magento Token Length",)
